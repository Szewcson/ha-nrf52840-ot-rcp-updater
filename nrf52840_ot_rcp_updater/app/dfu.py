...
class NrfDfuFlasher:
    ...
    def normal_usb_vid_pid(self, device: Path) -> str | None:
        """Return the normal RCP USB identity for diagnostics, never selection."""

        parent = self._usb_parent_for_tty(device)
        if parent is None:
            return None
        try:
            vendor = (parent / "idVendor").read_text(encoding="ascii").strip().lower()
            product = (parent / "idProduct").read_text(encoding="ascii").strip().lower()
        except OSError:
            return None
    ...
    def backend_available(self) -> bool:
        """Report whether the confined DFU helper is present before an operation."""
