...
class WorkflowSecurityTests(unittest.TestCase):
    ...
    def test_publication_requires_signed_artifacts_and_release_evidence(self) -> None:
        ...
        self.assertIn("reset_existing_firmware", workflow)
        self.assertIn("reset-firmware-branch:", publish)
        self.assertIn("firmware_signing_legacy_public_key.pem", publish)
        self.assertIn('git -C "${store}" rm -r --ignore-unmatch -- "${FIRMWARE_DIRECTORY}"', publish)
