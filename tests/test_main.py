...
from app.models import Artifact, FirmwareRelease, NcpVersion, ValidationError, ncs_dfu_application_version
from app.mqtt_update import MqttError
    ...
    def test_final_mqtt_failure_keeps_a_verified_installed_state(self) -> None:
        ...
        class FakeMqttEntity:
            def __init__(self, *args: object, **kwargs: object) -> None:
                del args
                self._submit_install = kwargs["submit_install"]
...
class MainPolicyDowngradeTests(unittest.TestCase):
    def test_explicit_mqtt_install_marks_a_lower_policy_target_as_selected(self) -> None:
        current = NcpVersion(
            raw="OPENTHREAD/current; NRF52840 PCA10059 N/3.5.0-preview1 Z/4.5.0",
            hardware="PCA10059",
            ncs_version="3.5.0-preview1",
            zephyr_version="4.5.0",
        )
        expected = NcpVersion(
            raw="OPENTHREAD/target; NRF52840 PCA10059 N/3.3.4 Z/4.3.99",
            hardware="PCA10059",
            ncs_version="3.3.4",
        ...
        release = FirmwareRelease(
            ...
            ncs_version="3.3.4",
            zephyr_version="4.3.99",
            dfu_application_version=ncs_dfu_application_version("3.3.4"),
            artifact=Artifact(
                "https://example.invalid/rcp.elf",
                "0" * 64,
                "rcp.elf",
            ...
            release_url="https://example.invalid/release",
            release_summary="Pinned release",
        )
        handlers: dict[int, object] = {}
        selected_targets: list[bool] = []
        ...
        class FakeUpdater:
            ...
            def install(
                ...
                del selected_release, progress
                selected_targets.append(selected_target)
                self._state_store.save(
                    {"installed": asdict(expected), "verified_at": "2026-09-06T00:00:00+00:00"}
                )
                handler = handlers[main_module.SIGTERM]
                assert callable(handler)
                handler(main_module.SIGTERM, None)
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            StateStore(root).save(
                {"installed": asdict(current), "verified_at": "2026-09-06T00:00:00+00:00"}
            )
            options = root / "options.json"
            options.write_text(
                '{"device":"/dev/null","safe_update":false,"pinned_ncs_minor":"3.3"}',
                encoding="utf-8",
            )
            environment = {
                "OT_RCP_OPTIONS": str(options),
            ...
            with (
                ...
                patch("app.main.RcpUpdater", FakeUpdater),
                patch("app.main._load_release", return_value=(object(), release, None)),
                patch("app.main._available_releases", return_value=(release,)),
            ):
                main_module.run()

        self.assertEqual(selected_targets, [True])
