...
class ConfigSchemaTests(unittest.TestCase):
    ...
    def test_exposes_the_admin_only_ingress_panel_without_a_host_port(self) -> None:
        config = _CONFIG.read_text(encoding="utf-8")

        self.assertIn("ingress: true", config)
        self.assertIn("ingress_port: 8099", config)
        self.assertIn("ingress_stream: false", config)
        self.assertIn("panel_icon: mdi:chip", config)
        self.assertIn("panel_title: PCA10059 RCP", config)
