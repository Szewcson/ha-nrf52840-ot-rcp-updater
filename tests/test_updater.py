...
def _firmware(directory: Path) -> Path:
    """Create one structurally valid PCA10059 ELF for transaction tests."""

    payload = b"NRF52840 PCA10059 N/3.4.0 Z/4.4.0"
    header = bytearray(116 + len(payload))
    ...
    struct.pack_into(
        "<IIIIIIII", header, 84, 1, 116, 0x1000, 0x1000, len(payload), len(payload), 5, 4
    )
    header[116:] = payload
    path = directory / "rcp.elf"
    path.write_bytes(header)
    return path


def _prepared_firmware(directory: Path, source: str = "upload") -> PreparedFirmware:
    path = _firmware(directory)
...
class _Flasher:
    ...
    def normal_usb_vid_pid(self, device: Path) -> str:
        del device
        return "1915:0000"
...
class SafeUpdateFallbackTests(unittest.TestCase):
    def test_preflight_allows_only_a_confirmed_bootloader_recovery_target(self) -> None:
        with TemporaryDirectory() as directory:
            flasher = _Flasher()
            updater = RcpUpdater(
                _settings(
            ...
            preflight = updater.preflight(_prepared_firmware(Path(directory)))

        self.assertTrue(preflight["ready"])
        checks = {check["name"]: check for check in preflight["checks"]}
        self.assertEqual(checks["RCP serial device"]["state"], "ok")
        self.assertIn("Secure DFU target is ready", checks["RCP serial device"]["message"])
        self.assertEqual(checks["bootloader transition"]["state"], "ok")
        self.assertEqual(flasher.selector_requests, [DfuSelector(None, "CC2180B1200E")])
    ...
    def test_allows_an_already_stopped_otbr_without_rest_api_access(self) -> None:
        ...
        with TemporaryDirectory() as directory:
            ...
            with patch("app.updater.download_artifact", return_value=_firmware(Path(directory))):
    ...
    def test_rejects_a_missing_rcp_and_dfu_before_stopping_otbr(self) -> None:
        ...
        with TemporaryDirectory() as directory:
            ...
            with (
                patch(
                    "app.updater.download_artifact", return_value=_firmware(Path(directory))
                ) as download,
    ...
    def test_rejects_if_otbr_state_changes_between_preflight_and_handoff(self) -> None:
        with TemporaryDirectory() as directory:
            ...
            with (
                patch("app.updater.download_artifact", return_value=_firmware(Path(directory))),
...
class NrfDfuUpdateTests(unittest.TestCase):
    def test_direct_prepared_artifact_uses_the_existing_transaction_and_revalidates_bytes(self) -> None:
        before = NcpVersion(
        ...
        with TemporaryDirectory() as directory:
            root = Path(directory)
            package = _prepared_firmware(root)
            flasher = _Flasher()
            updater = RcpUpdater(
                _settings(),
                StateStore(root),
            ...
            self.assertEqual(updater.install_prepared(package, selected_target=True), expected)
            self.assertEqual(flasher.flash_requests[0][0], package.path)
            self.assertEqual(
                flasher.flash_requests[0][2], ncs_dfu_application_version("3.4.0")
            )

            changed = _prepared_firmware(root)
            changed.path.write_bytes(changed.path.read_bytes() + b"changed")
            supervisor = _Supervisor("stopped")
            rejected = RcpUpdater(
                _settings(), StateStore(root), supervisor=supervisor, flasher=_Flasher()
            )
            with self.assertRaisesRegex(UpdateError, "artifact changed"):
                rejected.install_prepared(changed, selected_target=True)
            self.assertEqual(supervisor.temporarily_stop_calls, 0)
    ...
    def test_keeps_the_transaction_lock_through_durable_verified_state(self) -> None:
        ...
        with TemporaryDirectory() as directory:
            ...
            def progress(percentage: int | None, stage: str) -> None:
                del stage
                if percentage == 100:
                    try:
    ...
    def test_reconciles_a_verified_firmware_after_nrfdfu_reports_a_transfer_error(self) -> None:
        ...
        with TemporaryDirectory() as directory:
            firmware = _firmware(Path(directory))
    ...
    def test_flashes_the_manifest_elf_and_verifies_the_result(self) -> None:
        ...
        self.assertEqual(
            progress,
            [
                (None, "Validating release"),
                (None, "Downloading firmware"),
                (None, "Preparing OTBR and RCP"),
                (None, "Entering Secure DFU"),
                (None, "Flashing firmware"),
                (None, "Verifying RCP firmware"),
                (100, "Update verified"),
            ],
        )
    ...
    def test_qemu_workaround_settles_before_each_first_probe(self) -> None:
        ...
        self.assertIn((None, "Waiting for Secure DFU USB re-enumeration"), progress)
        self.assertIn((None, "Waiting for RCP USB re-enumeration"), progress)
